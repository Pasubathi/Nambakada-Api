...
async function pollDepGraph(id, orgId) {
    let attempts = 0;
    const maxAttempts = 50;
    while (attempts < maxAttempts) {
        try {
            const response = await polling_test_1.getDepGraph(id, orgId);
            return response.data.attributes;
        }
        catch (e) {
            await common_1.delayNextStep(attempts, maxAttempts, 1000);
            attempts++;
        }
    }
    ...
    for (const [path, scanResults] of Object.entries(scans)) {
        ...
        for (const scanResult of scanResults) {
            try {
                ...
                const { start_time, dep_graph_data, component_details, } = await pollDepGraph(id, orgId);
...
const api_token_1 = __webpack_require__(95181);
const common_2 = __webpack_require__(74434);
const common_3 = __webpack_require__(70527);
...
async function getDepGraph(id, orgId) {
    ...
    while (waitingTimeMs <= maxWaitingTimeMs) {
        try {
            await common_3.sleep(waitingTimeMs);
            result = await promise_1.makeRequest(payload);
            break;
        }
        catch (e) {
